...
impl FastEncoding for Fp {
    fn fast_encode(&self, data: &mut [u8]) -> Result<(), &'static str> {
        if data.len() != 32 {
            return Err("Invalid buffer length for encoding Fp!");
        }
...
impl FastDecoding for Fp {
    fn fast_decode(data: &[u8]) -> Result<Self, &'static str> {
        if data.len() != 32 {
            return Err("Invalid buffer length for decoding Fp!");
        }
        Ok(Fp([
...
impl ff::Field for Fp {
    ...
    fn sqrt_ratio(num: &Self, div: &Self) -> (Choice, Self) {
        ...
        #[cfg(all(feature = "static-sqrt-table", not(feature = "sqrt-table")))]
        {
            STATIC_FP_TABLES.sqrt_ratio(num, div)
        }
    ...
    #[cfg(all(feature = "static-sqrt-table", not(feature = "sqrt-table")))]
    fn sqrt_alt(&self) -> (Choice, Self) {
        STATIC_FP_TABLES.sqrt_alt(self)
    }
    ...
    fn sqrt(&self) -> CtOption<Self> {
        ...
        #[cfg(all(feature = "static-sqrt-table", not(feature = "sqrt-table")))]
        {
            let (is_square, res) = STATIC_FP_TABLES.sqrt_alt(self);
            CtOption::new(res, is_square)
        }
...
#[cfg(all(feature = "static-sqrt-table", not(feature = "sqrt-table")))]
const STATIC_FP_TABLES: StaticSqrtTables<Fp> =
    StaticSqrtTables::new(0x11BE, 1098, core::include_bytes!("../../data/fp.data"));
