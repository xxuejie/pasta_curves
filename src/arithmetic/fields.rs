...
pub trait SqrtTableHelpers: ff::PrimeField {
...
/// Trait for fast encoding a field
pub trait FastEncoding: ff::PrimeField {
    /// Encodes a field value to a buffer
    fn fast_encode(&self, data: &mut [u8]) -> Result<(), &'static str>;
}

/// Trait for fast decoding a field
pub trait FastDecoding: ff::PrimeField {
    /// Decodes a field value from a buffer
...
#[cfg_attr(docsrs, doc(cfg(feature = "sqrt-table")))]
#[derive(Debug)]
pub struct SqrtTables<F: SqrtTableHelpers> {
    hasher: SqrtHasher<F>,
    inv: Vec<u8>,
    g0: Box<[F; 256]>,
    g1: Box<[F; 256]>,
...
/// An abstract trait so both sqrt tables can share the implementations of
/// sqrt_ratio and sqrt_alt
#[cfg(any(feature = "sqrt-table", feature = "static-sqrt-table"))]
pub trait SqrtOperations<F: SqrtTableHelpers> {
    ...
    fn sqrt_ratio(&self, num: &F, div: &F) -> (Choice, F) {
        // Based on:
...
#[cfg(feature = "sqrt-table")]
impl<F: SqrtTableHelpers> SqrtOperations<F> for SqrtTables<F> {
    /// Common part of sqrt_ratio and sqrt_alt: return their result given v = u^((T-1)/2) and uv = u * v.
    fn sqrt_common(&self, uv: &F, v: &F) -> F {
        let sqr = |x: F, i: u32| (0..i).fold(x, |x, _| x.square());
...
#[cfg(feature = "sqrt-table")]
impl<F: SqrtTableHelpers + FastEncoding> SqrtTables<F> {
    /// Dump the derived data of SqrtTables to a buffer, so we can reuse
    /// the data in a StaticSqrtTables
    pub fn dump(&self) -> Result<Vec<u8>, &'static str> {
        let mut result = Vec::with_capacity((256 + 256 + 256 + 129) * 32 + self.inv.len());
        for v in &self.g0[..] {
            let mut buf = [0u8; 32];
            v.fast_encode(&mut buf)?;
...
#[cfg(feature = "static-sqrt-table")]
pub(crate) struct StaticSqrtTables<'a, F: SqrtTableHelpers> {
    hasher: SqrtHasher<F>,
    data: &'a [u8],
}

#[cfg(feature = "static-sqrt-table")]
impl<'a, F: SqrtTableHelpers + FastDecoding> StaticSqrtTables<'a, F> {
    #[allow(dead_code)]
    pub const fn new(hash_xor: u32, hash_mod: usize, data: &'a [u8]) -> Self {
        assert!(data.len() == Self::length(hash_mod));
    ...
    #[allow(dead_code)]
    pub const fn length(hash_mod: usize) -> usize {
        (256 + 256 + 256 + 129) * 32 + hash_mod
    }

    pub const fn inv(&self, idx: usize) -> u8 {
        self.data[(256 + 256 + 256 + 129) * 32 + idx]
    }

    pub fn g0(&self, idx: usize) -> F {
        F::fast_decode(&self.data[idx * 32..idx * 32 + 32]).expect("decoding g0")
    }

    pub fn g1(&self, idx: usize) -> F {
        F::fast_decode(&self.data[256 * 32 + idx * 32..256 * 32 + idx * 32 + 32])
            .expect("decoding g1")
    }

    pub fn g2(&self, idx: usize) -> F {
        F::fast_decode(&self.data[(256 + 256) * 32 + idx * 32..(256 + 256) * 32 + idx * 32 + 32])
            .expect("decoding g2")
    }

    pub fn g3(&self, idx: usize) -> F {
        F::fast_decode(
            &self.data[(256 + 256 + 256) * 32 + idx * 32..(256 + 256 + 256) * 32 + idx * 32 + 32],
        )
        .expect("decoding g3")
    }
}
...
#[cfg(feature = "static-sqrt-table")]
impl<'a, F: SqrtTableHelpers + FastDecoding> SqrtOperations<F> for StaticSqrtTables<'a, F> {
    /// Common part of sqrt_ratio and sqrt_alt: return their result given v = u^((T-1)/2) and uv = u * v.
    fn sqrt_common(&self, uv: &F, v: &F) -> F {
        let sqr = |x: F, i: u32| (0..i).fold(x, |x, _| x.square());
        let inv = |x: F| self.inv(self.hasher.hash(&x)) as usize;
...
#[cfg(feature = "ckb-asm")]
#[allow(unsafe_code)]
pub(crate) mod ckb_asm {
    #[inline(always)]
    pub fn widening_mul_256(a: &[u64; 4], b: &[u64; 4]) -> [u64; 8] {
